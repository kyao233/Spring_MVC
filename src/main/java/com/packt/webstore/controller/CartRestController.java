...
public class CartRestController {
	...
	public void addItem(@PathVariable String productId, HttpServletRequest request) {
		...
		logger.info(String.format("add item request session ID: %s ", sessionId));
		Product product = productService.getProductById(productId);
		if (product == null) {
			throw new IllegalArgumentException(new ProductNotFoundException("current productId does not exist!"));
		}
	...
	public void removeItem(@PathVariable String productId, HttpServletRequest request) {
		...
		String sessionId = request.getSession(true).toString();
		Cart cart = cartService.read(sessionId);
		if (cart == null) {
			cart = cartService.create(new Cart(sessionId));
		}
...
//	public @ResponseBody String getCardId (HttpServletRequest request) {
